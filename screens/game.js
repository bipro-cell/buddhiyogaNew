...
const [postIdCurrent,setPostID]=useState(551);
  const diceFace = useRef(Dice7);
//   const [diceFace,setDice]=useState(Dice7);
  const diceFaceFrame = useRef(null);
//   const [diceFaceFrame,setDiceFaceFrame]=useState(null);
  const cellInfoNow=useRef(null);
  const [gameState,setGameState] =useState(0);

//   const cellInfo = useRef("Loading please wait...");
  const [cellInfo,setCellInfo] = useState("Loading please wait...");
...
function changePage ()
{
  navigation.navigate('Posts',{postId:postIdCurrent})
}
...
//   cellInfo.current=data;
  ...
  const diceRoll=()=>{
    ...
    // setDice(diceImage[dice.iDiceCurrentRoll-1].imageurl);
    ...
    //   setDiceFaceFrame(imgProps)
    }
    else
    {

      diceFace.current=diceImage[6].imageurl;
        // setDice(diceImage[6].imageurl);
  ...
  const initiatePawnMovement = async ()=>{
    ...
    if (iDisplacement < 999) {
      ...
      if ((iDisplacement != 0) && (iDisplacement < 999) ) {
        ...
        // setPlayer(player.targetPosition, player.position+iDisplacement);
    }
...
//    cellInfo.current=playerPositions[player.position].info.name;  
   setCellInfo(playerPositions[player.position].info.name)
  ...
  //  setCell(cellInfo.current);
    ...
    setPostID(playerPositions[player.position].postID);
